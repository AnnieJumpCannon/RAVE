...
We used a quadratic vectorizer for the giant star model, and set the regularization
hyperparameter $\Lambda$ in the same way described in Section \ref{sec:ms-model}.
The results are shown in Figure \ref{fig:set-giant-model-hyperparameters}, where
again the enveloped region represents the minimum and maximum change for any label
with respect to the unregularized case.  At the point of maximum improvement 
...
third panel).  Based on decreasing RMS deviation in the labels, we adopt 
$\Lambda = X.XX$ for our giant star model.
