...
($\teff$, $\logg$) and up to 15 individual abundances from the \aspcap\ 
\citep{aspcap_paper}.  A subset of these elements have atomic transitions in the 
\rave\ wavelength region: \ion{O}{1}, \ion{Mg}{1}, \ion{Al}{1}, \ion{Si}{1}, 
\ion{Ca}{2}, \ion{Ti}{1}, \ion{Fe}{1}, and \ion{Ni}{1}.  However, we 
excluded [Ti/H] from our abundance list because of systematics in the training
set \citep{Holtzman_2015,Hawkins_2016}.  Therefore we are left with nine labels 
in our giant star model: $\teff$, $\logg$, and seven elemental abundances.  
...
% For each spectrum we have results from three different models, described in the previous sections

% the artefacts show that the models poorly extrapolate outside the training set.
% the joint model shows that a single quadratic model is too simple to apply to the entire rave data

% we cannot just use reduced chi sq to exclude stars, nor can we just exclude things that are
% outside the range of values in the training set --> stars project back into the label space

% therefore we will use the joint model to inform us as to whether we should adopt
% results from the giant star model, the dwarf model, or a linear combination of the two.


% Show results compared to the joint model.
% Central density of points that agree well with the joint model.
% Sigmas chosen to scale both models so that the central densities are approximately isotropic.
% The artefacts in the previous figures appear here too, in strange locations.

% Equations for calculating weights

% Summing bsaed on weights

% Show HRD with mean weight fraction
