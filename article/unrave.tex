...
The \unrave\ catalog presented here, in conjunction with \tgas, will 
...
$\chi_r^2$ in our analysis $< 3$, and that the \texttt{QK} flag reported by
\citet{Kordopatis_2013} is zero, indicating no problems with quality control.
...
We only include stars with $\chi_r^2 < 3$ and $S/N > 10$, although the latter
cut removed only few stars because the average S/N in the \rave\ spectra for
these stars is relatively high ($\gtrsim{}50$). When considering the relative 
information content available in \rave\ (945~pixels in the near infrared with
$\mathcal{R} ~ 7{,}500$) compared to these literature studies (\stub{what snr, resolution, wavelength range and hipparcos parallaxes}, we consider the
...
we use this example to illustrate caution for studies making inferences based
on main-sequence dwarf stars.  The results from both the main-sequence star model, 
...
a crucial inference, as the metallicities for stars in our main-sequence
...
% What information have we extracted: almost maximal use of giant branch,
% lesss so for dwarfs.

% Caveats in the results: metal-poor dwarfs/sub-giants
% HETEROGENEOUS TRAINING SET
% UNSURE IF TWO ABUNDANCE SCALES ARE ON THE SAME SCALE --!
% METAL POOR ABUNDANCES
% recommended SNR and chi-sq etc
% weird projections in space. --> use morphological classifications to guide

% despite these limitations, this catalog is the largest??? of abundances.

% in the future we will do spectrophotometric distances to improve on this
...
\caption{Stellar parameter ($\teff$, $\logg$, [Fe/H]) comparison between the fourth internal data release from the \ges\ survey, and this work. The number of stars in each panel are shown, as well as the bias and RMS deviation in each label. Stars are colored by the S/N of the \rave\ spectra, and only stars with $\chi_r^2 < 3$ and S/N $> 10$ are shown.  Most of the \ges-\rave\ overlap stars have relatively low S/N ratios in \rave, near $\approx 30$.\label{fig:ges-stellar-parameters}}
\end{figure*}
