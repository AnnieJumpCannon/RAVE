...
\newcommand{\apogee}{\project{\acronymn{APOGEE}}}
\newcommand{\lamost}{\project{\acronymn{LAMOST}}}
...
This premise relies on the ability to measure --- for large numbers of stars ---
3D positions and motions, and their detailed chemical abundances.  On 14 September
2016 the \gaia\ mission will release the former for $\approx2\times10^6$ stars.
However, a catalog of precise chemical abundances to complement that data
...
operates in:  8410--8795~\AA.  This setup includes the \ion{Ca}{2} 
...
There are no spectroscopic studies that have a large enough overlap with \rave, and
also cover our required range of stellar parameters (e.g., FGKM-type stars).  Moreover,
most of the spectroscopic studies we considered exhibit a flat lower main-sequence, a
consequence of violating the assumption of local thermal equilibrium 
\citep[e.g., see][for a discussion]{Bensby_2014}.  For these reasons we chose to make
use of the \epic\ catalog \citep{Huber_2016} for the training set labels.  The \epic\
catalog follows from the successful \project{Kepler} input catalog \citep{Brown_2011},
and provides probabilistic stellar classifications for $\gtrsim180{,}000$ stars in 
the \project{K2} fields based on the astrometric, photometric, and spectroscopic
information available.  There are 4,611 stars that overlap between \epic\ and \rave.


When the \epic\ posteriors are narrow (i.e., the uncertainties on labels are small),
\citet{Huber_2016} show that the labels are accurate and trustworthy.  However,
...
classification.  For this reason there are a number of stars that are mis-classified
in the \epic\ catalog.  \citet{Huber_2016} acknowledges this, noting: the fraction 
of X classified as Y could be as much as Z\%; up to Z2\% of X2 could be marked as Y2, 
and \stub{other things}.  In other words, the \epic\ labels are usually of high 
fidelity, and mis-classifications are clearly detectable if spectra are available.
As we will show, when the mis-classifications can be identified from \rave\ spectra, 
the \epic\ catalog makes a suitable training set for the main-sequence and sub-giant
branch in \rave.


% Identifying obviously bad stars that have been mis-classified by K2/EPIC

% Giant stars of low metallicity.

% Refer to the training set figure.
% Two separate models, with an overlap region.
% Worry later about label offsets.
% Make sure you mention the lack of abundance information in K2/EPIC





% Initially we train on three labels.
