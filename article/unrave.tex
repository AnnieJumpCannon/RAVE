...
\stub{We trained the model initially with no regularization. Could reproduce general structure of a main-sequence and HRD, but the intricacies of the structure was gone. Model appeared to be too simple. However we can use this model to identify dwarfs/giants,etc. We may revisit this model later}


\subsubsection{A 3-label model for the main-sequence}
...
spaced evenly in logarithmic space between $\Lambda = 10^{-3}$ to $\Lambda = 10^{3}$.
...
in Figure \ref{fig:hrd-from-individual-models}.  Significant structure is 
...
of stars near the giant branch clump is visible, which is likely due to
giant stars existing in the test set, but not in the training set.  In
later sections we will describe how our results from multiple models were
compiled to produce a single catalog.


\subsubsection{A 3-label model for the main-sequence and giant branch}
\label{sec:the-joint-model}

% Chose the regularization hyperparameter.
...
\ion{O}{1}, \ion{Mg}{1}, \ion{Al}{1}, \ion{Si}{1}, \ion{Ca}{2}, \ion{Ti}{1}, 
\ion{Fe}{1}, and \ion{Ni}{1}.  We excluded [Ti/H] from our abundance list 
...
% ARC asks: Should we discuss any tests done with censoring? 
%  			Do we feel like we need to defend our choice to use regularization
%           over censoring? The key result from the censoring tests that I did
%			is that there are a lot of atomic transitions here, but they are
% 			mostly blended. We *need* those blended lines because if you
%			censor all but the strongest line(s) for individual elements then
% 			you are left with a poor predictor of abundance labels.
