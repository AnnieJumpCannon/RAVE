...
We have performed an independent analysis of \rave\ spectra using \thecannon,
and here we report effective temperature $\teff$, surface gravity $\logg$, 
as well as chemical abundances of up to seven elements (O, Mg, Al, Si, Ca, Fe, 
Ni) for \Nstars\ stars.  We validate our results internally from repeat visits, 
and externally against existing high-resolution spectroscopic surveys.  The 
typical precision is XXX~K in $\teff$, X.XX~dex in $\logg$, and X.XX~dex for
detailed abundances.  We provide spectrophotometric distances based on our
labels, improving upon the parallax-based distances available for distant 
giants in the first \gaia\ data release.
...
in very low signal-to-noise (S/N) data or metal-poor stars.  Atomic transitions of 
...
analysis began.  First, we calculated inverse variance arrays from the $1\sigma$ 
...
step and the \emph{test} step.  These stages are briefly described in the
following section, and a more thorough introduction can be found in \citet{Ness_2015}.
We make the following explicit assumptions about the \rave\ spectra and \thecannon:
...
is invariant with respect to the labels we seek to measure, and invariant with 
respect to the S/N ratio.
...
afford to have a small fraction of inaccurate labels (e.g., a few obvious 
...
variance at the $j$-th wavelength pixel. 


At the \emph{training} step we fix the $K$-lists of labels for all the training set stars $n$.
...
\ref{eq:vectorizer-three-label}).  In practice equation \ref{eq:train} is 
...
be a useful training set for our purposes.  The thirteenth data release from the Sloan
Digital Sky Survey \citep{sloan_dr13} includes labels for \apogee\ stars on the
...
determinations ([Fe/H] $> -5$); S/N ratios of $>20)$ in \apogee\ $>25$ in \rave; and 
we further required that the \aspcap\ pipeline did not report any peculiar flags 
(\texttt{ASPCAPFLAG = 0}).  These restrictions left us with 357 stars along the giant 
branch, with metallicities ranging from $[{\rm Fe/H}] = -1.62$ to 0.32.  Intermediate 
...
practically useful.  Therefore, as we will describe in more detail below, we 
supplemented this sample with giant stars from the \epic\ catalog \citep{Huber_2016}.


Assembling a suitable training set for the main-sequence and sub-giant branch was less
trivial.  There are no spectroscopic studies that have a large enough sample size that
overlap with \rave\ and expand our required range in stellar parameters (e.g., FGKM-type 
stars).  Moreover, most of the spectroscopic studies we considered also showed a flat 
lower main-sequence, a systematic consequence of the analysis method adopted 
\citep[e.g., see][for a discussion]{Bensby_2014}.  For these reasons we chose to make 
...
usually of high fidelity, and any spurious misclassifications could be identified
if spectra were available.
...
(ignored) those quality constraints for stars with $[{\rm Fe/H}] < -1$.  After training
...
\epic\ for stars on the main-sequence and the sub-giant branch.  In the following section 
we will construct separate models for the giant branch and main-sequence.  We will
demonstrate the abundance labels are on the same scale, before combining the results from
multiple models to produce a single catalog.
...
The model we introduced in Section~\ref{sec:the-model} is generic in the sense that there 
is no description for how complex the vectorizing function $\vecv(\ell)$ should be, or how
many $K$ labels are used.  Given previous successes using a second-order (quadratic) 
polynomial model \citep{Ness_2015,Ness_2016,Ho_2016,Casey_2016b}, we will first adopt a 
three-label ($\teff$, $\logg$, [Fe/H]) model for all (main-sequence and giant) stars. Thus, 
for our first set of model experiments the vectorizer $\vecv(\ell_n)$ expands as,
...
treated all \Nspectra\ spectra as test set objects.  In Figure \ref{fig:3label-test-set}
we show the two-dimensional distribution of effective temperatures $\teff$ and surface 
gravities $\logg$ for all spectra with $\chi_{r}^2 < 3$ and $S/N > 10$.  The main-sequence
and red giant branch are clearly visible.  However, the details of stellar evolution are
no longer present: the sub-giant branch is not discernible, and there are a number of
systematic artefacts (over-densities) in the label space of the test set.  These artefacts
disappear when we require additional quality constraints (e.g., no peculiar morphological
classifications), but the details of the Hertzsprung-Russell diagram are still not
present.  Thus, we concluded that while this model is useful for discerning stellar
classifications (e.g., F2-type giant), the labels are too imprecise.


We chose to adopt a separate model for the main-sequence and the giant branch rather
than switch to a single, higher order model.  This choice allows us to infer
detailed abundance labels for giant stars, and to infer metallicity labels for stars
on the main-sequence and sub-giant branch.  However, adopting two separate models
introduces the challenge of how to combine the results from two models, or how to
assign one star as being a main-sequence/giant star.  In Section~\ref{sec:joining-the-models}
we describe how we will use the 3-label model of the main-sequence and giant branch
(introduced in this section) to discriminate between results from the 3-label 
main-sequence model in Section \ref{sec:unevolved-star-model} and the 9-label giant 
...
We inferred labels for all \rave\ spectra using this main-sequence model.  
The resulting labels for the survey sample are shown in Figure \ref{fig:hrd-from-individual-models}.  
...
way described in Section \ref{sec:unevolved-star-model}, using the same 30 trials of $\Lambda$.
...
\acknowledgements
We thank Jonathan Bird (Vanderbilt), Sven Buder (MPIA), and Sergey Koposov (Cambridge).
