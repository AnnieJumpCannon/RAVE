...
In practice this is achieved by fixing $s_j^2 = 0$ to make equation~\ref{eq:train} a
convex optimization problem, then we optimize for $\vectheta_j$, before solving for $s_j^2$.
We can also adopt strict priors on $\vectheta$ such that labels can only make non-zero
contributions to the data in certain wavelength ranges.  That is to say, we can specify
that certain labels (e.g., [Al/H]) can only contribute to the model at wavelength pixels
where we know an aluminium transition exists.  This method, which we call \emph{censoring},
is described in Section~\ref{sec:censoring} and \citet{Ness_2016b}. 
...
$\ell$, which provides us with the formal errors on $\ell$.  We judge the veracity of these
formal errors in our validation experiments in Section \ref{sec:validation}.  


\subsection{Three-label models ($\teff$, $\logg$, [Fe/H])}
\label{sec:a-simple-model}

% Given previous successes, we will first adopt a three-parameter model for giants + dwarfs.
% Polynomial thing --> expand out \vectheta
...
\subsection{A 10-label model for detailed abundances of giant stars}
